param ($function="automaticGamesUpdate", $maxgames=0, $maxpages=100, $id="", [switch] $prod=$false)
...
Invoke-Expression "./node_modules/.bin/env-cmd -f .env.dev node local_update.js --function $function --maxGames $maxgames --maxPages $maxpages $(If ($id) {'--id ' + $id} Else {''}) $(If ($prod) {'--prod'} Else {''}) 2>&1 | Tee-Object -a $logfile"
